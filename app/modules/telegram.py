...
class TelegramNotifier:
    ...
    def _build_watch_message(
        ...
        msg = (
            ...
            f"{signals_block}"
            f"{ttl_line}\n\n"
            f"\U0001f6e1\ufe0f <i>Auto-tracking active \u2014 you will be notified when:</i>\n"
            f"<i>  \u2022 \U0001f6a8 Entry zone is hit</i>\n"
            f"<i>  \u2022 \U0001f4b0 TP1/TP2/TP3 targets are reached</i>\n"
            f"<i>  \u2022 \U0001f6d1 Stop loss is triggered</i>\n"
            f"<i>\U0001f552 {ts}</i>"
        )
        return msg
    ...
    # ═══════════════════════════════════════════════════════════════════
    # V18: SIGNAL LIFECYCLE ENGINE — Entry/TP/SL/Expiry notifications
    # ═══════════════════════════════════════════════════════════════════
    ...
    async def send_entry_hit(
        ...
        """V18: ENTRY HIT — price entered the entry zone. Action alert."""
        from datetime import datetime, timezone
        ...
        zone_str = ""
        if entry_zone_low > 0 and entry_zone_high > 0:
            zone_str = f"\nZone: <b>${entry_zone_low:,.6f} \u2013 ${entry_zone_high:,.6f}</b>"

        tp_str = f"\nTP1: <b>${tp1_price:,.6f}</b>" if tp1_price > 0 else ""
        sl_str = f"\nSL: <b>${stop_loss:,.6f}</b>" if stop_loss > 0 else ""
        lev_str = f"\nLeverage: <b>{leverage}x</b>" if leverage > 1 else ""

        ts = datetime.now(timezone.utc).strftime("%H:%M UTC")

        msg = (
            f"\U0001f6a8 <b>ENTRY HIT \u2014 {symbol}</b>\n"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\n"
            f"Side: <b>{direction}</b>\n"
            ...
            f"{tp_str}"
            f"{sl_str}\n\n"
            f"\u26a0\ufe0f <b>Price is in the entry zone NOW!</b>\n"
            f"<i>Consider entering if setup is still valid.</i>\n"
            f"<i>\U0001f552 {ts}</i>"
        )
        await self.send(msg)

    async def send_trade_active(
        self,
        symbol: str,
        side: str,
        confidence: int,
        ...
        """V18: TRADE ACTIVE — entry confirmed, now tracking TP/SL."""
        from datetime import datetime, timezone
        ...
        tier_str = ""
        if quality_tier:
            tier_emoji = {"ELITE": "\U0001f451", "STRONG": "\U0001f537", "NORMAL": "\U0001f538", "WEAK": "\u26aa"}.get(quality_tier, "")
            tier_str = f"\nTier: <b>{tier_emoji} {quality_tier}</b>"

        # TP block
        tp_lines = []
        if tp1_price > 0:
            tp_lines.append(f"\U0001f3af TP1: <b>${tp1_price:,.6f}</b>")
        if tp2_price > 0:
            tp_lines.append(f"\U0001f3af TP2: <b>${tp2_price:,.6f}</b>")
        if tp3_price > 0:
            tp_lines.append(f"\U0001f3af TP3: <b>${tp3_price:,.6f}</b>")
        tp_block = "\n".join(tp_lines) if tp_lines else ""

        sl_str = f"\n\U0001f6d1 SL: <b>${stop_loss:,.6f}</b>" if stop_loss > 0 else ""
        rr_str = f"\n\nR:R = <b>1:{risk_reward:.1f}</b>" if risk_reward > 0 else ""

        ts = datetime.now(timezone.utc).strftime("%H:%M UTC")

        msg = (
            f"\u2705 <b>TRADE ACTIVE \u2014 {symbol}</b>\n"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\n"
            f"Side: <b>{direction}</b>\n"
            ...
            f"{sl_str}"
            f"{rr_str}\n\n"
            f"\U0001f6e1\ufe0f <b>Monitoring TP/SL automatically</b>\n"
            f"<i>Will notify on TP1/TP2/TP3 hit or SL hit.</i>\n"
            f"<i>\U0001f552 {ts}</i>"
        )
        await self.send(msg)
    ...
    async def send_tp_hit(
        ...
        """V18: TP HIT — take profit level reached."""
        from datetime import datetime, timezone
        ...
        tp_emoji = {
            "TP1": "\U0001f4b0",  # 💰
            "TP2": "\U0001f4b0\U0001f4b0",
            "TP3": "\U0001f389",  # 🎉
        }.get(tp_level, "\U0001f4b0")

        # Action hint
        if tp_level == "TP1":
            action = (
                f"\n\n\U0001f6e1\ufe0f <b>SL moved to breakeven</b>"
                + (f"\nNew SL: <b>${new_sl:,.6f}</b>" if new_sl > 0 else "")
                + "\n<i>Position is now risk-free. Holding for TP2/TP3.</i>"
            )
        elif tp_level == "TP2":
            action = "\n\n<i>Trailing remaining position toward TP3.</i>"
        else:
            action = "\n\n\u2705 <b>Full target reached! Signal complete.</b>"

        ts = datetime.now(timezone.utc).strftime("%H:%M UTC")

        msg = (
            f"{tp_emoji} <b>{tp_level} HIT \u2014 {symbol}</b>\n"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\n"
            f"Side: <b>{direction}</b>\n"
            f"Entry: <b>${entry_price:,.6f}</b>\n"
            f"{tp_level}: <b>${tp_price:,.6f}</b>\n"
    ...
    async def send_sl_hit(
        ...
        """V18: SL HIT — stop loss triggered."""
        from datetime import datetime, timezone
        ...
        msg = (
            ...
            f"Current: <b>${current_price:,.6f}</b>\n"
            f"P&L: <b>{pnl_pct:+.2f}%</b>\n\n"
            f"\u274c <b>Signal closed at stop loss.</b>\n"
            f"<i>\U0001f552 {ts}</i>"
        )
        await self.send(msg)
    ...
    async def send_signal_expired(
        ...
        """V18: SIGNAL EXPIRED — entry never triggered within validity window."""
        from datetime import datetime, timezone

        direction = "\U0001f7e2 LONG" if side == "BUY" else "\U0001f534 SHORT"
        age_min = int(age_seconds / 60) if age_seconds > 0 else 0
        ttl_min = expiry_seconds // 60 if expiry_seconds > 0 else 0
        ...
        msg = (
            ...
            f"Side: <b>{direction}</b>\n"
            f"Age: <b>{age_min}m</b>"
            + (f" (limit: {ttl_min}m)" if ttl_min > 0 else "")
            + f"\n\n"
            f"<i>Entry zone was never reached. No trade taken.</i>\n"
            f"<i>\U0001f552 {ts}</i>"
        )
        await self.send(msg)

    async def send_signal_invalidated(
        self,
        symbol: str,
        side: str,
        ...
        """V18: SIGNAL INVALIDATED — structure broke, setup no longer valid."""
        from datetime import datetime, timezone
        ...
        msg = (
            f"\u274c <b>SIGNAL INVALIDATED \u2014 {symbol}</b>\n"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"
            f"\u2500\u2500\n"
            f"Side: <b>{direction}</b>\n"
            f"Invalidation: <b>${invalidation_price:,.6f}</b>\n"
            f"Current: <b>${current_price:,.6f}</b>\n\n"
            f"<i>Setup structure broken. Signal cancelled.</i>\n"
            f"<i>\U0001f552 {ts}</i>"
        )
        await self.send(msg)
