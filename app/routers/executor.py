...
async def _register_signal_direct(
    ...
    # [SIGNAL DEBUG] Entry checkpoint
    logger.info(
        "[SIGNAL GENERATED] source=%s symbol=%s action=%s conf=%s price=%s",
        source, symbol, action, raw_conf, price
    )
    ...
    try:
        ...
        logger.error("[TELEGRAM SEND FAILED] %s %s exception: %s", signal_id_label, symbol, e)

    if telegram_sent:
        logger.info("[TELEGRAM SEND SUCCESS] %s %s - delivered to Telegram", signal_id_label, symbol)
    else:
        logger.warning("[TELEGRAM SEND FAILED] %s %s - returned False; check token/chat_id or HTTP error above", signal_id_label, symbol)
